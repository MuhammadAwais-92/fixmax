...
//            ['db' => 'deleted_at', 'dt' => 'deleted_at'],
        ...
        DataTable::where('page_type', '=', 'offer');
...
//        $perPage = ($page * $perPage) - $perPage;
        ...
        if (sizeof($pages['data']) > 0) {
            ...
            foreach ($pages['data'] as $key => $page) {
                ...
                if (!empty($trashedPages)) {
                    $pages['data'][$key]['actions'] = '<a class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill restore-record-button" href="javascript:{};" data-url="' . route('admin.dashboard.offers.restore', $page['id']) . '" title="Restore Page"><i class="fa fa-fw fa-undo"></i></a>' . '<span class="m-badge m-badge--danger">' . Carbon::parse($page['deleted_at'])->format($dateFormat) . '</span>';

                } else {
                    $pages['data'][$key]['actions'] = '<a href="' . route('admin.dashboard.offers.edit', $page['id']) . '" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit"><i class="fa fa-fw fa-edit"></i></a>'
                      . '<a class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill delete-record-button" href="javascript:{};" data-url="'. route('admin.dashboard.offers.destroy', $page['id']).'" title="Delete"><i class="fa fa-fw fa-trash-o"></i></a>';
                }
    ...
    public function save(OfferUpdateDto $params)
    {
        DB::beginTransaction();
        try {
    ...
    public function get($id = 0)
    {
        ...
        foreach (cache('LANGUAGES') as $lang) {
            ...
            if (is_null($page->name)) {
                $page->name = [];
                $page->name += [$lang['short_code'] => ''];
            } else {
                if (!array_key_exists($lang['short_code'], $page->name)) {
            ...
            if (is_null($page->content)) {
                $page->content = [];
                $page->content += [$lang['short_code'] => ''];
            } else {
                if (!array_key_exists($lang['short_code'], $page->content)) {
    ...
    public function all()
    {
        if ($this->getPaginate() == 0) {
            return $this->getModel()->where('page_type','offer')->all();
        } else {
            return $this->getModel()->where('page_type','offer')->paginate($this->getPaginate());
        }
