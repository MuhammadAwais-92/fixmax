<div class="uploder-ss-m">
    @if($required)
        <input type="text" name="images" class="d-none" required id="initial_images" style="opacity: 0;">
    @endif
    <div class="input-style1">
        <label class="d-block input-label">{{__($imageTitle) }} @if($isRequired) <span
                class="text-danger">*</span>@endif</label>
        <label class="d-block text-danger">{{__("Recommended Size 555 × 295")}}</label>
    </div>
    <div id="multiple" class="upload-sec d-flex align-items-center flex-wrap">
        <!-- image upload -->

        <div class="qust-filed" id="push_loader">
            <div class="form-control d-flex align-items-center justify-content-center" id="previous_html">
                ...
                <label for="choose-file-{{$imageNumber}}"
                    ...
                    <span class="js-fileName heading Poppins-Regular">{{__('Add Media')}}</span>
                </label>
            </div>
        </div>
        ...
        @if(!empty($displayImageSrc))
            ...
            @forelse($displayImageSrc as $key => $src)
                ...
                @if (str_contains($src['file_path'], '.mp4'))
                    ...
                    <div class="more-view-image">
                        ...
                        @if($src['file_default'] == 1 || $src['file_default'] == '1'|| $src['file_default'] == true)
                            <span class="d-flex align-items-center justify-content-center placeholder-remove-2">
...
@push('scripts')
    <script>
        ...
        $(document).on('change', '#choose-file-' + '{{ $imageNumber }}', function () {
            ...
            <input type="hidden" class="user-image selected-image default-img" name="{{$inputName}}[` + index + `][file_default]" value="0">
                            ...
                            if (image_count >= 1 && !image_def_boolean) {
                                let selected_el = $("[name='service_images[0][file_default]']");
                                selected_el.val(1);
        ...
        $(document).on('click', '.uploads', function () {
            ...
            let htm = '<span class="d-flex align-items-center justify-content-center placeholder-remove-2"><i class="fas fa-check"></i></span>';
